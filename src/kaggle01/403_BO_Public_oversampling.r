...
# se hace oversampling de los { BAJA+1, BAJA+2 }
...
PARAM$hs <- makeParamSet(
  makeIntegerParam("minsplit", lower = 400L, upper = 6000L),
  makeIntegerParam("minbucket", lower = 1500L, upper = 2000L),
  makeIntegerParam("maxdepth", lower = 9L, upper = 11L),
  makeIntegerParam("corte", lower = 9000L, upper = 10500L),
  forbidden = quote(minbucket > 0.5 * minsplit)
)
# minbuket NO PUEDE ser mayor que la mitad de minsplit

PARAM$semilla_azar <- 161729 # primera
...
# Oversampling agregando FISICAMENTE los registros a dtrain
#  no utilizo el parametro weights de la funcion rpart::rpart
# Para oversampling  32 + 32 +32
dbajas <- copy(dataset[ foto_mes==202103 & clase_ternaria %in% c("BAJA+1","BAJA+2" )])

for( i in 1:5 )  dbajas <- rbindlist( list( dbajas, dbajas) )
dbajas <- rbindlist( list(dbajas, dbajas, dbajas ) )

dtrain <- rbindlist( list(dtrain, dbajas) )
rm(dbajas)
gc()
